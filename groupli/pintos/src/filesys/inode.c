...
bool inode_resize(struct inode_disk *id, off_t size, block_sector_t origsector) {
  ...
  if (id->indirect == 0) {
    ...
    writer = origsector;
    indirectaddr = sector;
    ofs = sizeof(block_sector_t)+sizeof(off_t)+sizeof(unsigned)+123*sizeof(block_sector_t);
    cache_write(writer, &sector, sizeof(sector), ofs);
  }
  else {
...
//NERISSA
  ...
  if (id->doubleind == 0) {
    ...
    writer = origsector;
    indirectaddr = sector;
    ofs = sizeof(block_sector_t) + sizeof(off_t)+sizeof(unsigned)+123*sizeof(block_sector_t) + sizeof(block_sector_t);
    cache_write(writer, &sector, sizeof(sector), ofs);
  }
  else {
  ...
  while (i<128){
    ...
    if ((size > (123 + 128) * 512 + 512 * i * 128) && buffer[i] != 0){
      ...
      while (t<128){
        if ((size > (123 + 128) * 512 + 512 * i * 128 + t*512) && currbuf[t] == 0){
          if (free_map_allocate(1, &sector) == 0){
            inode_resize(id, id->length, origsector);
            free(currbuf);
            free(buffer);
    ...
    if ((size > (123 + 128) * 512 + 512 * i * 128) && buffer[i] == 0) {
      ...
      while (j<128){
        ...
        /*if ((size <= (123 + 128) * 512 + 512 * i * 128 + j * 512) && buffer2[j] != 0){
          free_map_release(buffer2[j], 1);
          buffer2[j] = 0;
        }*/

        if ((size > (123 + 128) * 512 + 512 * i * 128 + j * 512)) {
          if (free_map_allocate(1, &sector) == 0){
            inode_resize(id, id->length, origsector);
            free(buffer2);
            free(buffer);
