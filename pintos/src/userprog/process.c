...
process_execute (const char *file_name) 
{
  ...
  list_push_back(&parent->children,tid);
  printf("og tid: %d\n",tid);*/
  status[tid] = -2;
  //printf("%d\n",status[tid]);
  return tid;
...
{
  //if TID invalid or not child of calling process, return -1
 ...
 /* struct thread *cur = thread_current();
  ...
  for (e = list_begin (&cur->children); e != list_end (&cur->children); e = list_next (e)) {
      struct thread *t = list_entry (e, int, allelem);
      printf("child %d\n",t->tid);
      if (t==child_tid){
        return 100;
      }
