/* Grows a file from 0 bytes to 200*512 bytes, 512 bytes at a time,
...
test_main (void) 
{
  unsigned long long cur_read_cnt = block_read_cnt_call(); 
  unsigned long long cur_write_cnt = block_write_cnt_call();
  seq_test ("testfile",
            buf, sizeof buf, 0,
            return_block_size, check_file_size);
  CHECK (block_write_cnt_call() - cur_write_cnt > block_read_cnt_call() - cur_read_cnt, \
    "less reads than writes");
}
